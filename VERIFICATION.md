...
### ③ 손실을 겪으면 봇이 영원히 생각을 멈춘다 (구조적, 미수정)

`COLD_START_SAMPLES` 를 20에서 5로 낮춰 성과 하한을 시연에서 실제로
발동시키려다 발견했다. **낮추면 봇이 매매를 완전히 멈춘다.**

```
COLD_START_SAMPLES = 5
 회차 정산  cold  적중  평균수익   멈춘 지점                        체결
   4    4  True  0.50  +0.0200  THINK                            ●
   5    5 False  0.40  -0.0099  THINK                            ●
   6    5 False  0.40  -0.0099  가치있는 시그널 없음 — 지출 중단이 알파   ·
  ...  (15회 연속 동일)
```

원인은 `agents/pipeline.py` `should_think()` 다.
...
`avg_return` 이 음수가 되면 `est_value` 가 음수라 **항상 SKIP** 이다.
SKIP 하면 새 거래가 없고 → 새 정산이 없고 → `avg_return` 이 갱신되지 않는다.
**한 번 손실 구간에 들어가면 빠져나올 수 없는 교착이다.**

지금은 `COLD_START_SAMPLES = 20` 이 표본 20건을 요구해서 시연 길이에서는
드러나지 않는다. 즉 **이 값이 버그를 가리고 있다.**

- **영향**: 장기 자동 운전에서 봇이 조용히 멈춘다. 에러도 안 난다.
- **고치려면**: `avg_return` 을 0에서 클램프하거나(`max(0, ...)`),
  기대값이 음수일 때 탐색을 완전히 끊는 대신 최소 표본을 계속 쌓게 해야 한다.
- **이번에 고치지 않았다.** `should_think` 는 매수 판단의 핵심이라
  제출 직전에 바꾸면 검증을 처음부터 다시 해야 한다.
- **부작용으로 확인된 것**: 기본값 20에서 8사이클 후 멈추는 것은 버그가
  아니라 **일일 인지예산 상한이 작동한 것**이다.
  `일일 상한 초과: 500300 > 500000` — 사이클당 61,750µ × 8 = 494,000µ.
  "법인카드에 한도가 있다"의 실증이므로 시연에서 보여줄 만하다.
...
> **[2026-08 수정됨] `external-sale` 이 진짜 402를 타게 되었다.**
> 예전에는 이 단계가 `LEDGER.transfer` 직접 호출이었고, 코드 주석에도
> `# 외부 에이전트의 테제 구매 (모의)` 라고 적혀 있었다. 지금은 판매 창구
> `/bots/{id}/sell/thesis/{id}` 를 그대로 통과한다:
> ```
> x402  /bots/bot2/sell/thesis/ths_7615f0ed57ed
>       external → revenue-wallet@bot2   200,000 µUSDC   ✓증빙 소비
> ```
> 402 챌린지 → 정책 검사 → 결제 → 증빙 첨부 재요청 → 증빙 1회성 소비를
> 전부 밟는다. 위 표의 "직접 이체 / 외부의 테제 구매 (모의)" 행은
> 이제 **x402** 로 옮겨 읽어야 한다.
>
> 남은 한계: 구매자가 우리 프로세스 안의 `EXTERNAL_BUYER` 트래커다.
> 실제 서비스에서는 남의 서버가 그 자리에 오지만, **밟는 경로는 동일하다.**
> 그리고 영수증이 불완전하면(`degraded`) 창구가 409로 거절하고 사이클은
> 계속된다 — 판매가 안 된 것이지 판단이 틀린 게 아니기 때문.
